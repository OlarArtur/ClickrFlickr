...
class FlickrUserAuthentication {
    ...
    private static func getRequestToken(completion: @escaping (_ token: String?, _ secretToken: String?) -> ()) throws {
        ...
        guard let urlRequestToken = getRequestURL(neededOauthParameters: neededParameters, requestURL: Constants.requestTokenUrl) else {throw FlickOauthError.ResponseError}
